                ...
                if col not in df.columns:
                    ...
                    if cleaned_col in df.columns:
                        input_mapping[col_group] = \
                            list(map(lambda x: x.replace(col, cleaned_col), input_mapping[col_group]))
                    else:
...
def create_datetime_features(df, datetime_cols, include_hour=False, make_copy=False):
    ...
    for dt_col in datetime_cols:
        ...
        if include_hour: df.loc[:, 'hour'] = df[dt_col].dt.hour
        ...
        holidays = calendar().holidays(start=str(np.min(np.array(df[dt_col]))),
...
def create_customer_history(df,
    ...
    if isinstance(generate_lagging_mean_cols, (list, tuple)):
        ...
        if n_jobs_lag == 1:
            for feature in generate_lagging_mean_cols:
                df['mean_prev_' + feature] = \
                    df.groupby(customer_id_col)[feature] \
                        .transform(lambda x: x.shift().expanding().mean().fillna(0))
        else:
            def parallelize_lag(_df, feature):
                _df['mean_prev_' + feature] = \
                    _df.groupby(customer_id_col)[feature] \
                       .transform(lambda x: x.shift().expanding().mean().fillna(0))
                return _df
            ...
            df = reduce(lambda x, y: pd.merge(x, y[[i for i in y.columns if i not in x.columns]],
...
class CalcAmexMetrics:
    ...
    @staticmethod
    def create_metric_dict(y_true, y_pred, threshold=.5):
        ...
        output_dict = dict(amex_metric=CalcAmexMetrics.amex_metric(y_true, y_pred),
        ...
        if groupby_split:
            if len(melt_id_vars) == 1:
                melted_metrics_df = \
                    self.df.loc[:, all_needed_cols] \
                        .melt(id_vars=list(set(list(melt_id_vars) + [self.target])),
                              var_name='fit', value_name='value') \
                ...
                melted_metrics_df = \
                    self.df.loc[:, all_needed_cols] \
                        ...
                        .apply(lambda df: self.create_metric_dict(df[self.target], df['value'])) \
                        .pipe(lambda df: pd.concat([pd.DataFrame(df),
                                                    pd.json_normalize(pd.DataFrame(df)[0]) \
                                                   .set_index(pd.DataFrame(df).index)],
                                                   axis=1,
                                                   sort=False)) \
                        .drop(0, axis=1) \
            ...
            melted_metrics_df = \
                df[self.fits + [self.target]] \
                    ...
                    .pipe(lambda df:
                          pd.DataFrame.from_dict(self.create_metric_dict(df[self.target],
                                                                         df['value']),
                                                 orient='index')) \
...
def focal_loss(alpha=.25, gamma=1):
...
def amex_loss():
    ...
    def amex_metric(y_true: pd.Series, y_pred: pd.Series) -> float:
        ...
        def top_four_percent_captured(y_true: pd.DataFrame, y_pred: pd.DataFrame) -> float:
            ...
            df = (pd.concat([y_true_s, y_pred_s], axis='columns')
                  .sort_values(y_pred_s.name, ascending=False))
