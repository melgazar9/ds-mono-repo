...
STARTING_BANKROLL = 100000 # Not directly used in the current PNL calculation, but good for context
BET_AMOUNT = 2000
STOP_LOSS_AMOUNT = 4000
END_OF_DAY_TIME = dtime(14, 55) # 2:55 PM CST

DATA_DIR = os.path.expanduser("/home/melgazar9/scripts/dev/ds-mono-repo/projects/financial_strategies/htb_equities/data/")
...
def track_trades(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.loc[:, "no_stop_pnl"] = (df["trade_open_price"] - df["close"]) * df["no_stop_position_size"]
    ...
    def close_trade(group):
        if group["stop_triggered"].any():
            stop_idx = group["stop_triggered"].idxmax() # Get the index of the first stop trigger
            stop_value = group.loc[stop_idx, "no_stop_pnl"] # Get the PNL at the stop trigger
            group.loc[group.index >= stop_idx, "pnl_with_stop"] = stop_value
        else:
            group["pnl_with_stop"] = group["no_stop_pnl"] # If no stop, PNL is just no_stop_pnl
        ...
        group.loc[eod_mask, "stop_triggered"] = False # Stop is no longer active after EOD
        return group
...
for file_path in all_csv_files:
    ...
    df_pl = df_pl.with_columns([
        pl.col("window_start").cast(pl.Datetime(time_unit="ns")).dt.replace_time_zone("UTC").alias("window_start_utc"),
    ])

    df_pl = df_pl.with_columns([
        pl.col("window_start_utc").dt.convert_time_zone("America/New_York").alias("window_start_cst")
    ]).with_columns([
    ...
    for col in ["open", "high", "low", "close"]:
        df_pl = df_pl.with_columns([
            ...
            ((pl.col(col) - pl.col("prev_close_1d")) / pl.col("prev_close_1d")).alias(f"{col}_minus_prev_close_pct_chg")
        ])

    df_pl = df_pl.with_columns(
        (pl.col("high_minus_prev_close_pct_chg") > PCT_CHANGE_TRIGGER).alias("trigger_condition_pct")
    )
    ...
    df_pl = df_pl.with_columns(
        pl.col("time_cst").is_between(dtime(3, 0), dtime(13, 45)).alias("trigger_within_time_range")
    )
...
if all_processed_dfs:
    ...
    column_order = [
        "window_start", "window_start_utc", "window_start_cst", "time_cst",
        "date", "ticker", "open", "high", "low", "close", "volume", "dividends",
        "stock_splits", "repaired", "prev_close_5m",
        "open_minus_prev_close", "open_minus_prev_close_pct_chg",
        "high_minus_prev_close", "high_minus_prev_close_pct_chg",
        "low_minus_prev_close", "low_minus_prev_close_pct_chg",
        "close_minus_prev_close", "close_minus_prev_close_pct_chg",
        "trigger_condition_pct", "trigger_within_time_range",
        "no_stop_position_size", "trade_open_price", "no_stop_pnl",
        "can_enter_trade", "pnl_leq_stop", "stop_triggered", "pnl_with_stop",
        "flat_time"
    ]
    ...
    output_filename = f"harold_short_strategy_{datetime.now().strftime('%Y-%m-%d__%H-%M-%S')}.parquet"
    final_df.to_parquet(os.path.join(PROCESSED_DIR, output_filename))
    print(f"Processing complete. Results saved to: {os.path.join(PROCESSED_DIR, output_filename)}")
else:
