...
class SingleDayBacktest:
    ...
    def _trigger_entry_booleans(
        ...
        if isinstance(pre_market_pct_chg_range, tuple):
            assert (
                len(pre_market_pct_chg_range) == 2
            ), "Pre-market percentage change range must be a tuple of two floats."
            pmkt_pct_chg_lower, pmkt_pct_chg_upper = (
                pre_market_pct_chg_range[0],
                pre_market_pct_chg_range[1],
            )
            if direction == "up":
                self.df_cur["pmkt_trigger_entry"] = (
                    self.df_cur["adj_pmkt_pct_chg"] >= pmkt_pct_chg_lower
                ) & (self.df_cur["adj_pmkt_pct_chg"] <= pmkt_pct_chg_upper)
            elif direction == "down":
                self.df_cur["pmkt_trigger_entry"] = (
                    self.df_cur["adj_pmkt_pct_chg"] <= -pmkt_pct_chg_lower
                ) & (self.df_cur["adj_pmkt_pct_chg"] >= -pmkt_pct_chg_upper)
                ...
                self.df_cur["pmkt_trigger_entry"] = (
                    self.df_cur["adj_pmkt_pct_chg"]
                    .abs()
                    .between(pmkt_pct_chg_lower, pmkt_pct_chg_upper)
                )
        else:
        ...
        if isinstance(market_pct_chg_range, tuple):
            assert (
                len(market_pct_chg_range) == 2
            ), "Market percentage change range must be a tuple of two floats."
            mkt_pct_chg_lower, mkt_pct_chg_upper = (
                market_pct_chg_range[0],
                market_pct_chg_range[1],
            )
            if direction == "up":
                self.df_cur["mkt_trigger_entry"] = (
                    self.df_cur["adj_mkt_pct_chg"] >= mkt_pct_chg_lower
                ) & (self.df_cur["adj_mkt_pct_chg"] <= mkt_pct_chg_upper)
            elif direction == "down":
                self.df_cur["mkt_trigger_entry"] = (
                    self.df_cur["adj_mkt_pct_chg"] <= -mkt_pct_chg_lower
                ) & (self.df_cur["adj_mkt_pct_chg"] >= -mkt_pct_chg_upper)
                ...
                self.df_cur["mkt_trigger_entry"] = (
                    self.df_cur["adj_mkt_pct_chg"]
                    .abs()
                    .between(mkt_pct_chg_lower, mkt_pct_chg_upper)
                )
        else:
            self.df_cur["mkt_trigger_entry"] = False

        if trigger_condition == "and":
            assert isinstance(pre_market_pct_chg_range, tuple) and isinstance(
                market_pct_chg_range, tuple
            ...
            assert isinstance(pre_market_pct_chg_range, tuple) or isinstance(
                market_pct_chg_range, tuple
            ...
            assert isinstance(
                pre_market_pct_chg_range, tuple
    ...
    def trigger_buy_sell(
        ...
        first_high = (
            trigger_rows.groupby(["ticker", "date"])["high"].first().rename("bto_price")
        )
        self.df_cur = self.df_cur.merge(first_high, on=["ticker", "date"], how="left")

        first_low = (
            trigger_rows.groupby(["ticker", "date"])["low"].first().rename("sto_price")
        )
        self.df_cur = self.df_cur.merge(first_low, on=["ticker", "date"], how="left")
        return self

    def trigger_stop_loss(self, stop_loss_pct: float = 0.01):
        """
        ...
        assert isinstance(
            stop_loss_pct, float
        ), "Stop-loss percentage must be a percentage (float)."
        ...
        self.df_cur["raw_short_stop"] = (
            self.df_cur["high"] >= self.df_cur["stc_stop_price"]
        )

        self.df_cur["long_stop_triggered"] = self.df_cur.groupby(["ticker", "date"])[
            "raw_long_stop"
        ].transform(first_true)
        self.df_cur["short_stop_triggered"] = self.df_cur.groupby(["ticker", "date"])[
            "raw_short_stop"
        ].transform(first_true)
    ...
    def trigger_take_profit(
        ...
        assert (
            isinstance(take_profit_pct, float) and 0 < take_profit_pct < 1
        ), "Take-profit percentage must be a float between 0 and 1."
        assert (
            isinstance(slippage_pct, float) and 0 < slippage_pct < 1
        ), "Slippage percentage must be a float between 0 and 1."
        assert slippage_method in (
            "partway",
            "fixed_pct",
        ...
        self.df_cur["long_tp_triggered"] = self.df_cur.groupby(["ticker", "date"])[
            "high"
        ].transform(lambda x: first_true(x >= self.df_cur.loc[x.index, "btc_tp_price"]))
        self.df_cur["short_tp_triggered"] = self.df_cur.groupby(["ticker", "date"])[
            "low"
        ].transform(lambda x: first_true(x <= self.df_cur.loc[x.index, "stc_tp_price"]))

    def calc_rolling_pnl_no_stop_no_profit(
        self, slippage_pct=0.10, slippage_method="partway", min_slip=0.01
    ):
        """
        ...
        if slippage_method == "fixed_pct":
            ...
            long_entry_price_with_slippage = (
                bto + np.maximum(high - bto, min_slip) * slippage_pct
            )
            short_entry_price_with_slippage = (
                sto + np.minimum(low - sto, -min_slip) * slippage_pct
            )
            long_exit_price_with_slippage = (
                close - np.maximum(close - low, min_slip) * slippage_pct
            )
            short_exit_price_with_slippage = (
                close + np.maximum(high - close, min_slip) * slippage_pct
            )
        else:
            raise ValueError("slippage_method must be 'fixed_pct' or 'partway'")

        self.df_cur["long_pnl_no_stop"] = (
            long_exit_price_with_slippage - long_entry_price_with_slippage
        )
        self.df_cur["short_pnl_no_stop"] = (
            short_entry_price_with_slippage - short_exit_price_with_slippage
        )

    def calc_pnl_stop_only(
        self, slippage_pct=0.10, slippage_method="partway", min_slip=0.01
    ):
        """
        ...
        # EXIT at stop
        long_exit = np.where(
            low <= btc_stop_price, np.minimum(btc_stop_price, low), np.nan
        )
        short_exit = np.where(
            high >= stc_stop_price, np.maximum(stc_stop_price, high), np.nan
        )

        self.df_cur["long_pnl_stop_only"] = long_exit - long_entry
        self.df_cur["short_pnl_stop_only"] = short_entry - short_exit

    def calc_pnl_tp_only(
        self, slippage_pct=0.10, slippage_method="partway", min_slip=0.01
    ):
        """
    ...
    def calc_pnl_with_stop_and_tp(self):
        ...
        long_first_trigger_idx = self.df_cur.groupby(
            ["ticker", "date"], group_keys=False
        ).apply(lambda g: first_trigger_idx(g, long_stop_hit, long_tp_hit, long_both))
        short_first_trigger_idx = self.df_cur.groupby(
            ["ticker", "date"], group_keys=False
        ...
        self.df_cur.loc[long_first_exit_mask & long_stop_hit, "long_pnl_stop_or_tp"] = (
            self.df_cur.loc[long_first_exit_mask & long_stop_hit, "long_pnl_stop_only"]
        )
        self.df_cur.loc[
            long_first_exit_mask & ~long_stop_hit & long_tp_hit, "long_pnl_stop_or_tp"
        ] = self.df_cur.loc[
            long_first_exit_mask & ~long_stop_hit & long_tp_hit, "long_pnl_tp_only"
        ]

        self.df_cur.loc[
            short_first_exit_mask & short_stop_hit, "short_pnl_stop_or_tp"
        ] = self.df_cur.loc[short_first_exit_mask & short_stop_hit, "short_pnl_stop_only"]
        self.df_cur.loc[
            short_first_exit_mask & ~short_stop_hit & short_tp_hit, "short_pnl_stop_or_tp"
        ] = self.df_cur.loc[
    ...
    def trigger_exit(self, close_trade_timestamp_cst=time(14, 55)):
        ...
        """
        assert isinstance(
            close_trade_timestamp_cst, time
        ), "close_trade_timestamp_cst must be a datetime.time object."
        self.df_cur["close_timestamp_triggered"] = (
            self.df_cur["timestamp_cst"].dt.time >= close_trade_timestamp_cst
        )
        self.df_cur["exit_long"] = (
            self.df_cur["long_stop_triggered"]
            | self.df_cur["long_tp_triggered"]
            | self.df_cur["close_timestamp_triggered"]
        )
        self.df_cur["exit_short"] = (
            self.df_cur["short_stop_triggered"]
            | self.df_cur["short_tp_triggered"]
            | self.df_cur["close_timestamp_triggered"]
        )

        self.df_cur["exit_long_timestamp"] = np.where(
            self.df_cur["exit_long"], self.df_cur["timestamp_cst"], pd.NaT
        )
        self.df_cur["exit_short_timestamp"] = np.where(
            self.df_cur["exit_short"], self.df_cur["timestamp_cst"], pd.NaT
        )
        return self
    ...
    def integrate_segments(self):
        """
        Method to integrate segments to find optimal subgroups of the strategy (e.g. short_volume, short_interest,
        market_cap, etc.)
        """
        pass
    ...
    def run_backtest(self, cur_day_file: str, prev_day_file: str):
        ...
        self.trigger_stop_loss(stop_loss_pct=0.01)
        self.trigger_take_profit(
            take_profit_pct=0.01, slippage_pct=0.1, slippage_method="partway"
        )
