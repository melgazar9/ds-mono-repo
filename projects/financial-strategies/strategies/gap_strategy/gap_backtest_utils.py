...
class GapPositionManager(PositionManager):
    ...
    def detect_trade(self, df: Union[pd.DataFrame, pl.DataFrame]) -> Union[pd.DataFrame, pl.DataFrame]:
        logging.info("🔍Detecting Trade Opportunities")
        df.loc[
            (df["adj_pmkt_pct_chg"] >= self.overnight_gap) & (df["timestamp_cst"].dt.time <= self.entry_cutoff_time_cst),
            "trigger_trade_entry_pmkt_raw",
        ] = True

        df.loc[
            (df["adj_mkt_pct_chg"] >= self.overnight_gap) & (df["timestamp_cst"].dt.time <= self.entry_cutoff_time_cst),
            "trigger_trade_entry_mkt_raw",
        ] = True

        df["trigger_trade_entry_pmkt"] = df.groupby(["ticker", "date"])["trigger_trade_entry_pmkt_raw"].transform(first_true)
        df["trigger_trade_entry_mkt"] = df.groupby(["ticker", "date"])["trigger_trade_entry_mkt_raw"].transform(first_true)

        df["trigger_trade_entry"] = df["trigger_trade_entry_pmkt"] | df["trigger_trade_entry_mkt"]
        df["trigger_trade_entry"] = df.groupby(["ticker", "date"])["trigger_trade_entry"].transform(first_true)

        df["trigger_trade_entry"] = df["trigger_trade_entry"].fillna(False)

        df = df.drop(columns=["trigger_trade_entry_pmkt_raw", "trigger_trade_entry_mkt_raw"])
    ...
    def _open_position(self, df: Union[pd.DataFrame, pl.DataFrame]):
        ...
        df["first_pmkt_trade_timestamp"] = (
            df[df["timestamp_cst"].dt.time < dtime(8, 30)].groupby(["ticker", "date"]).cumcount() == 0
        )
        df["first_mkt_trade_timestamp"] = (
            df[df["timestamp_cst"].dt.time >= dtime(8, 30)].groupby(["ticker", "date"]).cumcount() == 0
        )
        df["first_pmkt_bar"] = df[df["timestamp_cst"].dt.time == dtime(3, 0)].groupby(["ticker", "date"]).cumcount() == 0

        df.loc[df["trigger_trade_entry"], "theoretical_bto_price"] = df["prev_market_close"] * (1 + self.overnight_gap)
        ...
        df["bto_price_with_slippage"] = df.groupby(["ticker", "date"])["bto_price_with_slippage"].ffill()
        df["sto_price_with_slippage"] = df.groupby(["ticker", "date"])["sto_price_with_slippage"].ffill()

        df.loc[df["trigger_trade_entry"], "shares_long_first"] = self.bet_amount / df["bto_price_with_slippage"]
        df.loc[df["trigger_trade_entry"], "shares_short_first"] = self.bet_amount / df["sto_price_with_slippage"]
    ...
    def _trigger_stop_loss(self, df: Union[pd.DataFrame, pl.DataFrame]):
        ...
        df["long_stop_limit_price_raw"] = df["bto_price_with_slippage"] * (1 - self.stop_loss_pct)
        df["long_stop_hit_raw"] = df["low"] <= df["long_stop_limit_price_raw"]
        df["long_stop_hit"] = df.groupby(["ticker", "date"])["long_stop_hit_raw"].transform(first_true)

        df["long_stop_limit_price"] = df["long_stop_limit_price_raw"].where(df["long_stop_hit"])

        df["long_stop_hit"] = df["long_stop_hit"].fillna(False)

        # Short stop logic
        df["short_stop_limit_price_raw"] = df["sto_price_with_slippage"] * (1 + self.stop_loss_pct)
        df["short_stop_hit_raw"] = df["high"] >= df["short_stop_limit_price_raw"]
        df["short_stop_hit"] = df.groupby(["ticker", "date"])["short_stop_hit_raw"].transform(first_true)
        df["short_stop_limit_price"] = df["short_stop_limit_price_raw"].where(df["short_stop_hit"])

        df["short_stop_hit"] = df["short_stop_hit"].fillna(False)

        df = df.drop(
            columns=["long_stop_limit_price_raw", "long_stop_hit_raw", "short_stop_limit_price_raw", "short_stop_hit_raw"]
        )
    ...
    def _trigger_take_profit(self, df: Union[pd.DataFrame, pl.DataFrame]):
        ...
        df["long_tp_limit_price_raw"] = df["bto_price_with_slippage"] * (1 + self.take_profit_pct)
        df["long_tp_hit_raw"] = df["high"] >= df["long_tp_limit_price_raw"]
        df["long_tp_hit"] = df.groupby(["ticker", "date"])["long_tp_hit_raw"].transform(first_true)
        df["long_tp_hit_ffilled"] = df.groupby(["ticker", "date"])["long_tp_hit"].ffill()

        df["long_tp_limit_price"] = df["long_tp_limit_price_raw"].where(df["long_tp_hit"])

        df["long_tp_hit"] = df["long_tp_hit"].fillna(False)

        # Short tp logic
        df["short_tp_limit_price_raw"] = df["sto_price_with_slippage"] * (1 - self.take_profit_pct)
        df["short_tp_hit_raw"] = df["low"] <= df["short_tp_limit_price_raw"]
        df["short_tp_hit"] = df.groupby(["ticker", "date"])["short_tp_hit_raw"].transform(first_true)
        df["short_tp_hit_ffilled"] = df.groupby(["ticker", "date"])["short_tp_hit"].ffill()
        df["short_tp_limit_price"] = df["short_tp_limit_price_raw"].where(df["short_tp_hit"])

        df["short_tp_hit"] = df["short_tp_hit"].fillna(False)

        df = df.drop(columns=["long_tp_limit_price_raw", "long_tp_hit_raw", "short_tp_limit_price_raw", "short_tp_hit_raw"])
    ...
    def _add_exit_prices(self, df):
        ...
        df["long_position_active"] = df.groupby(["ticker", "date"])["has_long_entry"].transform("cummax")
        df["short_position_active"] = df.groupby(["ticker", "date"])["has_short_entry"].transform("cummax")
        ...
        df["is_last_bar"] = df.groupby(["ticker", "date"]).cumcount(ascending=False) == 0
        ...
        df.loc[df["long_tp_hit"], "long_exit_priority"] = np.minimum(df.loc[df["long_tp_hit"], "long_exit_priority"], 2)
        df.loc[df["short_tp_hit"], "short_exit_priority"] = np.minimum(df.loc[df["short_tp_hit"], "short_exit_priority"], 2)

        # Stop losses (highest priority - can overwrite everything)
        df.loc[df["long_stop_hit"], "long_exit_priority"] = np.minimum(df.loc[df["long_stop_hit"], "long_exit_priority"], 1)
        df.loc[df["short_stop_hit"], "short_exit_priority"] = np.minimum(df.loc[df["short_stop_hit"], "short_exit_priority"], 1)

        # Get first exit per group
        df["long_exit_rank"] = df.groupby(["ticker", "date"])["long_exit_priority"].rank(method="first")
        df["short_exit_rank"] = df.groupby(["ticker", "date"])["short_exit_priority"].rank(method="first")

        # Apply exit prices based on final priority
        first_long_exit = (df["long_exit_rank"] == 1) & (df["long_exit_priority"] < np.inf)
        first_short_exit = (df["short_exit_rank"] == 1) & (df["short_exit_priority"] < np.inf)

        # Stop exits (priority 1)
        df.loc[first_long_exit & (df["long_exit_priority"] == 1), "long_exit_price"] = df.loc[
            first_long_exit & (df["long_exit_priority"] == 1), "long_stop_exit_price"
        ]
        df.loc[first_short_exit & (df["short_exit_priority"] == 1), "short_exit_price"] = df.loc[
            first_short_exit & (df["short_exit_priority"] == 1), "short_stop_exit_price"
        ]

        # TP exits (priority 2)
        df.loc[first_long_exit & (df["long_exit_priority"] == 2), "long_exit_price"] = df.loc[
            first_long_exit & (df["long_exit_priority"] == 2), "long_tp_exit_price"
        ]
        df.loc[first_short_exit & (df["short_exit_priority"] == 2), "short_exit_price"] = df.loc[
        ...
        if eod_long_exits.any():
            df.loc[eod_long_exits, "long_exit_price"] = apply_slippage(
                df.loc[eod_long_exits], price_col="open", slippage_amount=0.005, slippage_mode="fixed_pct", side="sell"
            )

        if eod_short_exits.any():
            df.loc[eod_short_exits, "short_exit_price"] = apply_slippage(
                df.loc[eod_short_exits], price_col="open", slippage_amount=0.005, slippage_mode="fixed_pct", side="buy"
            )

        # === ADD EXIT TIMESTAMPS ===

        # Set exit timestamps for all exits
        df.loc[first_long_exit, "long_exit_timestamp"] = df.loc[first_long_exit, "timestamp_cst"]
        df.loc[first_short_exit, "short_exit_timestamp"] = df.loc[first_short_exit, "timestamp_cst"]

        # Get entry timestamps per group
        df["long_entry_timestamp"] = df.groupby(["ticker", "date"])["timestamp_cst"].transform(
            lambda x: (x[df.loc[x.index, "has_long_entry"]].iloc[0] if df.loc[x.index, "has_long_entry"].any() else pd.NaT)
        )
        df["short_entry_timestamp"] = df.groupby(["ticker", "date"])["timestamp_cst"].transform(
            lambda x: (x[df.loc[x.index, "has_short_entry"]].iloc[0] if df.loc[x.index, "has_short_entry"].any() else pd.NaT)
        )
        ...
        df.loc[first_long_exit, "time_in_long_trade"] = (
            df.loc[first_long_exit, "long_exit_timestamp"] - df.loc[first_long_exit, "long_entry_timestamp"]
        )
        df.loc[first_short_exit, "time_in_short_trade"] = (
            df.loc[first_short_exit, "short_exit_timestamp"] - df.loc[first_short_exit, "short_entry_timestamp"]
        )
    ...
    def _ffill_shares(df):
        ...
        df["long_shares_temp"] = df.groupby(["ticker", "date"])["shares_long_first"].ffill()

        # Create position active flag (from entry to exit)
        df["long_entry_cumsum"] = df.groupby(["ticker", "date"])["shares_long_first"].transform(lambda x: x.notna().cumsum())
        df["long_exit_cumsum"] = df.groupby(["ticker", "date"])["long_exit_price"].transform(lambda x: x.notna().cumsum())

        # Position is active from first entry until first exit
        long_position_active = (df["long_entry_cumsum"] >= 1) & (df["long_exit_cumsum"] == 0)
        ...
        df.loc[long_position_window, "long_shares_ffilled"] = df.loc[long_position_window, "long_shares_temp"]

        # === SHORT POSITIONS ===
        df["short_shares_temp"] = df.groupby(["ticker", "date"])["shares_short_first"].ffill()

        df["short_entry_cumsum"] = df.groupby(["ticker", "date"])["shares_short_first"].transform(lambda x: x.notna().cumsum())
        df["short_exit_cumsum"] = df.groupby(["ticker", "date"])["short_exit_price"].transform(lambda x: x.notna().cumsum())

        short_position_active = (df["short_entry_cumsum"] >= 1) & (df["short_exit_cumsum"] == 0)
        short_exit_bar = df["short_exit_price"].notna()
        short_position_window = short_position_active | short_exit_bar

        df.loc[short_position_window, "short_shares_ffilled"] = df.loc[short_position_window, "short_shares_temp"]
    ...
    def _add_unrealized_pnl(self, df):
        ...
        df["long_entry_price_temp"] = df.groupby(["ticker", "date"])["bto_price_with_slippage"].ffill()

        # Calculate PnL only where shares exist
        long_pnl_mask = df["long_shares_ffilled"].notna()
        df["unrealized_long_pnl"] = np.where(
            long_pnl_mask, (df["open"] - df["long_entry_price_temp"]) * df["long_shares_ffilled"], np.nan
        )

        # === SHORT PnL ===
        df["short_entry_price_temp"] = df.groupby(["ticker", "date"])["sto_price_with_slippage"].ffill()

        short_pnl_mask = df["short_shares_ffilled"].notna()
        df["unrealized_short_pnl"] = np.where(
            short_pnl_mask, (df["short_entry_price_temp"] - df["open"]) * df["short_shares_ffilled"], np.nan
        )

        # Clean up
        df.drop(columns=["long_entry_price_temp", "short_entry_price_temp"], inplace=True)

        return df

    @staticmethod
    def _add_realized_pnl(df):
        df.loc[:, "long_realized_pnl"] = (df["long_exit_price"] - df["bto_price_with_slippage"]) * df["long_shares_ffilled"]
        df.loc[:, "short_realized_pnl"] = (df["sto_price_with_slippage"] - df["short_exit_price"]) * df["short_shares_ffilled"]
        return df
    ...
    def adjust_position(self, df: Union[pd.DataFrame, pl.DataFrame]) -> Union[pd.DataFrame, pl.DataFrame]:
...
class GapStrategyRiskManager(RiskManager):
    def quantify_risk(self, df: Union[pd.DataFrame, pl.DataFrame]) -> Union[pd.DataFrame, pl.DataFrame]:
...
class GapStrategyEvaluator(StrategyEvaluator):
    ...
    def _calc_daily_summary(df, include_pct_chg_metrics=False):
        ...
        summary_dict = {
            ...
            "pct_entries_pmkt": (
                (df["trigger_trade_entry_pmkt"].sum() / (df["trigger_trade_entry_pmkt"] | df["trigger_trade_entry_mkt"]).sum())
                if has_trades
                else 0
            ),
            "pct_entries_mkt": (
                (df["trigger_trade_entry_mkt"].sum() / (df["trigger_trade_entry_pmkt"] | df["trigger_trade_entry_mkt"]).sum())
                if has_trades
                else 0
            ),
            # === GAP ANALYSIS ===
            ...
            "long_win_rate": (calc_win_rate(df["long_realized_pnl"]) if has_long_pnl else np.nan),
            "long_profit_factor": (calc_profit_factor(df["long_realized_pnl"].dropna()) if has_long_pnl else np.nan),
            ...
            "long_intraday_sharpe": (calc_intraday_sharpe_ratio(df["long_realized_pnl"]) if has_long_pnl else np.nan),
            "long_intraday_sortino": (calc_sortino_ratio(df["long_realized_pnl"]) if has_long_pnl else np.nan),
            "long_intraday_max_drawdown": (calc_max_drawdown(df["long_realized_pnl"]) if has_long_pnl else np.nan),
            # === SHORT POSITION METRICS ===
            "short_realized_pnl": safe_sum(df["short_realized_pnl"]),
            "short_win_rate": (calc_win_rate(df["short_realized_pnl"]) if has_short_pnl else np.nan),
            "short_profit_factor": (calc_profit_factor(df["short_realized_pnl"].dropna()) if has_short_pnl else np.nan),
            ...
            "short_intraday_sharpe": (calc_intraday_sharpe_ratio(df["short_realized_pnl"]) if has_short_pnl else np.nan),
            "short_intraday_sortino": (calc_sortino_ratio(df["short_realized_pnl"]) if has_short_pnl else np.nan),
            "short_intraday_max_drawdown": (calc_max_drawdown(df["short_realized_pnl"]) if has_short_pnl else np.nan),
            # === ENTRY/EXIT TIMESTAMPS ===
            "avg_trade_entry_timestamp": (safe_mean(df[df["trigger_trade_entry"]]["timestamp_cst"]) if has_trades else pd.NaT),
            "median_trade_entry_timestamp": (
                safe_median(df[df["trigger_trade_entry"]]["timestamp_cst"]) if has_trades else pd.NaT
            ),
            "earliest_trade_entry": (safe_min(df[df["trigger_trade_entry"]]["timestamp_cst"]) if has_trades else pd.NaT),
            "latest_trade_entry": (safe_max(df[df["trigger_trade_entry"]]["timestamp_cst"]) if has_trades else pd.NaT),
            "avg_long_exit_timestamp": (
                safe_mean(df[df["long_exit_price"].notna()]["timestamp_cst"]) if has_long_exits else pd.NaT
            ),
            "avg_short_exit_timestamp": (
                safe_mean(df[df["short_exit_price"].notna()]["timestamp_cst"]) if has_short_exits else pd.NaT
            ),
            "median_long_exit_timestamp": (
                safe_median(df[df["long_exit_price"].notna()]["timestamp_cst"]) if has_long_exits else pd.NaT
            ),
            "median_short_exit_timestamp": (
                safe_median(df[df["short_exit_price"].notna()]["timestamp_cst"]) if has_short_exits else pd.NaT
            ),
            # === TIME IN TRADE ===
            ...
            # === EXIT TYPE ANALYSIS ===
            "n_long_stop_exits": df[(df["long_exit_price"].notna()) & (df.get("long_exit_priority", pd.Series()) == 1)].shape[
                0
            ],
            "n_long_tp_exits": df[(df["long_exit_price"].notna()) & (df.get("long_exit_priority", pd.Series()) == 2)].shape[0],
            "n_long_eod_exits": df[(df["long_exit_price"].notna()) & (df.get("long_exit_priority", pd.Series()) == 3)].shape[0],
            "n_short_stop_exits": df[
                (df["short_exit_price"].notna()) & (df.get("short_exit_priority", pd.Series()) == 1)
            ].shape[0],
            "n_short_tp_exits": df[(df["short_exit_price"].notna()) & (df.get("short_exit_priority", pd.Series()) == 2)].shape[
                0
            ],
            "n_short_eod_exits": df[(df["short_exit_price"].notna()) & (df.get("short_exit_priority", pd.Series()) == 3)].shape[
                0
            ],
            # === CORPORATE ACTIONS ===
            "n_had_split": df[df["split_ratio"] != 1]["ticker"].nunique(),
            "n_had_dividend": df[df["cash_amount"] != 0]["ticker"].nunique(),
            "n_had_split_or_dividend": df[(df["split_ratio"] != 1) | (df["cash_amount"] != 0)]["ticker"].nunique(),
    ...
    def _calc_daily_summary_by_ticker(self, df):
        ...
        with warnings.catch_warnings():
            ...
            daily_summary_by_ticker = daily_summary_by_ticker.drop(columns=["date"]).reset_index()
    ...
    def _add_ticker_identifiers(df):
        ...
        assert rows_before_merge == df.shape[0], "additional rows joined after merging tickers!"
    ...
    def evaluate_strategy(self, df: Union[pd.DataFrame, pl.DataFrame]) -> Union[pd.DataFrame, pl.DataFrame]:
        logging.info("📈 Evaluating strategy...")
        rows_before = df.shape[0]
        df = self._add_pct_change_columns(df)
        df = self._add_segments(df)
        assert df.shape[0] == rows_before, "Mismatch in rows after adding segments in evaluation."
        ...
        if self.segment_cols:
            self.daily_summary_with_segments = self._calc_daily_summary_by_segment(df[df["ticker"].isin(tickers_traded)])
