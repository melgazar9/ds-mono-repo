...
class PullPriceData:
    ...
    def get_1min_bars(self):
        ...
        df_1min = self.bars_to_df(bars_1min)
        return df_1min
    ...
    def get_prev_3mo_history(self):
        ...
        df_90days = self.bars_to_df(bars_90d)
        return df_90days
...
def get_current_price(df_1min):
    return df_1min["close"].iloc[-1]
...
def compute_recommendation(ticker, min_avg_30d_volume=1500000, min_iv30_rv30=2.0, max_ts_slope_0_45=-0.0075):
    try:
        ...
        try:
            ...
            raise f"Error {e}: Not enough option data."

        options_chains = {}
        for exp_date in exp_dates:
            options_chains[exp_date] = stock.option_chain(exp_date)

        price_obj = PullPriceData(ticker=ticker, exchange="SMART", currency="USD")
        df_price_history = price_obj.get_prev_3mo_history()
