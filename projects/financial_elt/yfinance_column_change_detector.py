...
sys.path.append(f'{home_path}/scripts/github/personal/ds-mono-repo/projects/financial_elt/tap-yfinance/.meltano/extractors/tap-yfinance/venv/lib/python3.9/site-packages')
...
ignore_cols = ['timestamp', 'timestamp_tz_aware', 'timezone', 'ticker', '_sdc_extracted_at', '_sdc_received_at', '_sdc_batched_at', '_sdc_deleted_at', '_sdc_sequence', '_sdc_table_version', '_sdc_sync_started_at']
...
for table in tables:
    try:
        if table in ["option_chain", "shares_full"]:
            ...
            continue
        if table not in ["options", "quarterly_balance_sheet", "quarterly_cash_flow", "quarterly_financials", "quarterly_income_stmt"]:
            df_yfinance = getattr(t, f"get_{table}")()
        else:
            if table == 'options':
    ...
    try:
        # if table is wide
        df_missing_yf_i = pd.DataFrame({'column': [i for i in clean_strings(df_postgres.columns) if i not in clean_strings(df_yfinance.columns)]})
        df_missing_yf_i['table'] = table
        df_missing_yf_i = df_missing_yf_i[['table', 'column']]

        df_missing_db_i = pd.DataFrame({'column': [i for i in clean_strings(df_yfinance.columns) if i not in clean_strings(df_postgres.columns)]})
        df_missing_db_i['table'] = table
        df_missing_db_i = df_missing_db_i[['table', 'column']]
    except:
        try:
            # if table is long
            df_missing_yf_i = pd.DataFrame({'column': [i for i in clean_strings(df_postgres.columns) if i not in clean_strings(df_yfinance.index)]})
            df_missing_yf_i['table'] = table
            df_missing_yf_i = df_missing_yf_i[['table', 'column']]

            df_missing_db_i = pd.DataFrame({'column': [i for i in clean_strings(df_yfinance.index) if i not in clean_strings(df_postgres.columns)]})
            df_missing_db_i['table'] = table
            df_missing_db_i = df_missing_db_i[['table', 'column']]
        except:
