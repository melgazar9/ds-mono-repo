{{ config(schema='yf_clean', materialized='table', unique_key=['day', 'ticker']) }}
...
cte_ticker_intervals as (
  ...
  cross join
    (select distinct ticker from {{ ref('yf_futures_prices_1d') }}) t
),
