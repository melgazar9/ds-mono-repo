{{ config(schema='yf_clean', materialized='incremental', unique_key=['minute', 'ticker']) }}
...
cte_ticker_intervals as (
  ...
  cross join
    (select distinct ticker from {{ ref('yf_forex_prices_1m') }}) t
),
