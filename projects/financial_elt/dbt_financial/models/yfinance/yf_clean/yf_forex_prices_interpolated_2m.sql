{{ config(schema='yf_clean', materialized='incremental', unique_key=['two_minute', 'ticker']) }}
...
cte_ticker_intervals as (
  ...
  cross join
    (select distinct ticker from {{ ref('yf_forex_prices_2m') }}) t
),
