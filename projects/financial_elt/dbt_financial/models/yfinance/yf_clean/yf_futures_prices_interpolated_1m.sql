{{ config(schema='yf_clean', materialized='table', unique_key=['minute', 'ticker']) }}
...
cte_ticker_intervals as (
  ...
  cross join
    (select distinct ticker from {{ ref('yf_futures_prices_1m') }}) t
),
