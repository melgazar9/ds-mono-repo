{{ config(schema='yfinance_clean', materialized='table', unique_key=['five_minute', 'ticker']) }}
...
cte_ticker_intervals as (
  ...
  cross join
    (select distinct ticker from {{ ref('yf_crypto_prices_5m') }}) t
),
