...
cte_ticker_intervals as (
  ...
  cross join
    (select distinct ticker from {{ ref('yf_stock_prices_5m') }}) t
),
...
where
  ...
  {% if is_incremental() %}
    and ct.five_minute >= (select max(date(five_minute)) - interval '3 day' from {{ this }})
  {% endif %}
order by 1