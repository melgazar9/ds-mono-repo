...
class PostgresConnect(metaclass=MetaclassRDBMSEnforcer):
    ...
    NOTE - When you use an object in a with statement, Python looks for these two methods:
        __enter__: called when the with block is entered. When running "with PostgresConnect(...) as db:"
        the "db" value refers to the connected PostgresConnect instance.
    ...
    def run_sql(self, query, df_type="pandas", **read_sql_kwargs):
        ...
        **read_sql_kwargs: kwargs passed to pd.read_sql or pl.read_database.
        ...
        if is_batched_read and (self.con is None or self.con.closed):
            ...
            raise RuntimeError(
                "For batched reads (iter_batches=True), the PostgresConnect instance must be used within a 'with'"
                "statement or connect() must be called and managed manually."
            )
